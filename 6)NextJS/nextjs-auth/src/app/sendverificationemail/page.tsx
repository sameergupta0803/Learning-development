...
const SendVerifyEmail = () => {
  ...
  const onSendVerifyEmail = async () => {
    try {
      setLoading(true);
      const response = await axios.post("/api/users/sendverificationemail", {
        email,
      });
      toast.success(
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen py-2">
      {isEmailSent ? (
        ...
        <div className="flex flex-col items-center justify-center min-h-screen py-2">
          <h1>{loading ? "Sending verification email..." : "Verify Email"}</h1>
          <label
          ...
          >
            {buttonDisabled ? "Please fill the email" : "Send Verification Email"}
          </button>
